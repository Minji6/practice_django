...
var masonrys = document.getElementsByTagName('img'); //벽돌 형태로 엇비슷하기 올려가는 레이아웃 형태, 문서 내부에서 img 태그를 다 가져온다.

for (let i = 0; i < masonrys.length; i++) { //masonrys의 개수만큼 도는데
    masonrys[i].addEventListener('load', function() { //하나하나마다 addEventListener로 load가 되었을 때 function을 넣어주는데 이것이 magicgrid
        magicGrid.positionItems(); //magicGrid의 postionItems을 보여주고 끝낼 것이다.
    }, false); //모든 html 안에 있는 이미지가 로드되었을 때, 매직그리드를 다시 포지셔닝하라는 이벤트리스너를 추가한 것이다.
}
